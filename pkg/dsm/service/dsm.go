...
func (service *DsmService) DisconnectIscsiSessions(targetIqn string) error {
	for _, dsm := range service.dsms {
		targets, err := dsm.TargetList()
		if err != nil {
			continue
		}
		for _, target := range targets {
			...
			log.Infof("Enabling multiple sessions on target %s to recover from stale session", targetIqn)
			return dsm.TargetSet(strconv.Itoa(target.TargetId), 0)
		}
	}
